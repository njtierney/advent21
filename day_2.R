...
dive <- read_csv(file = "data/day_2.txt",
                 col_names = "course")
...
dive_summary <- dive_sep %>%
  group_by(direction) %>%
  summarise(amount = sum(amount))

dive_wider <- dive_summary %>%
  pivot_wider(
...
dive_depth <- dive_wider$down - dive_wider$up
dive_depth
dive_horizontal <- dive_wider$forward
dive_horizontal

dive_mult <- dive_depth * dive_horizontal
dive_mult
